...
namespace EscapedPrisoner
{
    [CalloutProperties("Escaped Prisoner", "GGGDunlix", "0.1.0")]
    public class EscapedPrisoner : Callout
    {
        Ped prisoner;
...
new Vector3(1673.594f, 2705.125f, 45.23011f),
        ...
        public EscapedPrisoner()
        {
            Vector3 location = coordinates.OrderBy(x => World.GetDistance(x, Game.PlayerPed.Position)).Skip(3).First();

            InitInfo(location);
            ShortName = "Escaped Prisoner";
            CalloutDescription = "A prisoner escaped from the Bolingbroke Penitentiary. Respond in Code 3.";
            ResponseCode = 3;
            StartDistance = 60f;
        }
